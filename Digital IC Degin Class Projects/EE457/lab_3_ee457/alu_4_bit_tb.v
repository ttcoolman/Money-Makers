...
task TEST_ALU;
 ...
 reg [16*8:1] Opr_string_tb;
   begin
	   ...
	   test_num = test_num + 1;
	   case (Opr_value)
	   		0	: if ((~AINV_value) && BNEG_value) Opr_string_tb = "SLT operation";
			...
			2	: if (AINV_value && BNEG_value) Opr_string_tb = "NAND operation";
				  else if (!(AINV_value || BNEG_value) ) Opr_string_tb = " OR operation";
				  else Opr_string_tb = "UNK operation"; // unknown operation
	        3	: if (AINV_value && BNEG_value) Opr_string_tb = "NOR operation";
...
initial
  ...
  file_results = $fopen("alu_output_results.txt", "w");
   test_num = 0;
// test #1 begin
	TEST_ALU (11, 7, 0, 0, 3); // (A,B,AINV,BNEG,Opr)  AND
// test #1 end

// test #2 begin
	TEST_ALU (11, 7, 0, 0, 2); // (A,B,AINV,BNEG,Opr)  OR
// test #2 end

// test #3 begin
	TEST_ALU (11, 7, 0, 0, 1); // (A,B,AINV,BNEG,Opr)  ADD
// test #3 end

// test #4 begin
	TEST_ALU (11, 7, 0, 1, 1); // (A,B,AINV,BNEG,Opr)  SUB
// test #4 end

// test #5 begin
	TEST_ALU (11, 7, 0, 1, 0); // (A,B,AINV,BNEG,Opr)  SLT
// test #5 end

// test #6 begin
	TEST_ALU (4, 7, 0, 1, 0); // (A,B,AINV,BNEG,Opr)  SLT
// test #6 end

// test #7 begin
	TEST_ALU (7, 4, 0, 1, 0); // (A,B,AINV,BNEG,Opr)  SLT
// test #7 end

// test #8 begin
	TEST_ALU (11, 12, 0, 1, 0); // (A,B,AINV,BNEG,Opr)  SLT
// test #8 end

// test #9 begin
	TEST_ALU (11, 10, 0, 1, 0); // (A,B,AINV,BNEG,Opr)  SLT
// test #9 end

// test #10 begin
	TEST_ALU (11, 7, 1, 1, 3); // (A,B,AINV,BNEG,Opr)  NOR
// test #10 end

// test #11 begin
	TEST_ALU (11, 7, 1, 1, 2); // (A,B,AINV,BNEG,Opr)  NAND
// test #11 end
